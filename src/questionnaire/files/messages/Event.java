...
public class Event extends Message {	
    ...
    public boolean obeyConditions(String fileName, String activismName, String[] politicalName,  
        ...
        byte deathCondition = 0;
        if (conditionDeath.item != null) {
            if (conditionDeath.item[0].trim().equals(deathName)) { // "death+" or "death-"
                deathCondition = 1;
            }
        } else {
            deathCondition = 1;
        ...
        System.out.println("Obey conditions (event): " + deathCondition);
        // TRUE if obeying condition, FALSE if not 
        if(!(deathCondition == 1)) auxObeyConditions = false;
